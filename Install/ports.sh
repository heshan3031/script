...
edit_squid () {
msg -azu "$(fun_trans ${id} "REDEFINIR PUERTOS SQUID")"
msg -bar
...
msg -ne "$(fun_trans ${id} "Nuevas Puertas"): "
read -p "" newports
...
sleep 1s
msg -bar
msg -azu "$(fun_trans ${id} "PUERTAS REDEFINIDAS")"
msg -bar
}
edit_apache () {
msg -azu "$(fun_trans ${id} "REDEFINIR PUERTAS APACHE")"
msg -bar
local CONF="/etc/apache2/ports.conf"
local NEWCONF="$(cat ${CONF})"
msg -ne "$(fun_trans ${id} "Nuevas Puerta"): "
read -p "" newports
for PTS in `echo ${newports}`; do
...
edit_openvpn () {
msg -azu "$(fun_trans ${id} "REDEFINIR PUERTAS OPENVPN")"
msg -bar
...
msg -ne "$(fun_trans ${id} "Nueva Puerta"): "
read -p "" newports
for PTS in `echo ${newports}`; do
...
edit_dropbear () {
msg -azu "$(fun_trans ${id} "REDEFINIR PUERTAS DROPBEAR")"
msg -bar
...
edit_openssh () {
msg -azu "$(fun_trans ${id} "REDEFINIR PUERTAS OPENSSH")"
msg -bar
...
for((a=1; a<=$i; a++)); do
[[ $squid = $a ]] && echo -ne "\033[1;32m [$squid] > " && msg -azu "$(fun_trans ${id} "REDEFINIR PUERTAS SQUID")"
[[ $apache = $a ]] && echo -ne "\033[1;32m [$apache] > " && msg -azu "$(fun_trans ${id} "REDEFINIR PUERTA APACHE")"
[[ $openvpn = $a ]] && echo -ne "\033[1;32m [$openvpn] > " && msg -azu "$(fun_trans ${id} "REDEFINIR PUERTA OPENVPN")"
[[ $dropbear = $a ]] && echo -ne "\033[1;32m [$dropbear] > " && msg -azu "$(fun_trans ${id} "REDEFINIR PUERTAS DROPBEAR")"
[[ $ssh = $a ]] && echo -ne "\033[1;32m [$ssh] > " && msg -azu "$(fun_trans ${id} "REDEFINIR PUERTAS SSH")"
done
echo -ne "\033[1;32m [0] > " && msg -azu "$(fun_trans ${id} "VOLVER")"
echo -e "$barra"
while true; do
