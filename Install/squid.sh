...
fun_squid  () {
  ...
  [[ -e $var_squid ]] && {
  echo -e "\033[33m $(fun_trans ${id} "REMOVIENDO SQUID")"
  msg -bar
  service squid stop > /dev/null 2>&1
  fun_bar "apt-get remove squid3 -y"
  echo -e "\033[33m $(fun_trans ${id} "Procedimento Concluido")"
  msg -bar
  [[ -e $var_squid ]] && rm $var_squid
  return 0
  }
echo -e "\033[33m $(fun_trans ${id} "INSTALADOR SQUID ADM-ULTIMATE")"
msg -bar
fun_ip
