...
ureset_python () {
for port in $(cat /etc/newadm/Python.log)
do
PIDVRF3="$(ps aux|grep proxycl-"$port" |grep -v grep|awk '{print $2}')"
if [[ -z $PIDVRF3 ]]; then
screen -dmS proxycl-"$port" python /etc/newadm/proxy-color.py "$port"
else
for pid in $(echo $PIDVRF3); do
echo""
done
...
pidproxy3=$(ps x | grep -w  "proxy-color.py" | grep -v "grep" | awk -F "pts" '{print $1}') && [[ ! -z $pidproxy3 ]] && P3="<span class='encendido'> ACTIVO " || P3="<span class='detenido'> DESACTIVADO | REINICIANDO "
echo "<p>Estado del servicio Proxy Python color  está ||  $P3 </span>.</p> " >> $DIR/$ARCHIVO
#LIBERAR RAM,CACHE
